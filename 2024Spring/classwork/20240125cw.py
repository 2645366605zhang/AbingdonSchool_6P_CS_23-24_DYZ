...
class Company:
  ...
  def ProcessDayEnd(self):
    ...
    if self.checkIfBankrupt():
      Details += "\nThe company is in the state of bankrupt."
    return Details
...
class Simulation:
  ...
  def ProcessDayEnd(self):
    ...
    for companyID in bankruptCompanies:
      self.closeCompany(companyID)
  ...
  def ProcessXDays(self) -> str:
    x = None
    while x == None:
      x = input("Please enter the number of days you wish to proceed.(within 30 days)\nEnter Q to quit\n")
      #print(f"x: {x}\nRange: {list(range(1, 30))}\nResult: {(int(x) in list(range(1, 31)))}")
      if x.upper() == "Q":
        return ""
      elif not(int(x) in list(range(1, 30))):
        x = None
    for frequency in range(int(x)):
      self.ProcessDayEnd()
    return "6"
  
  def closeCompany(self, ID) -> None:
    print(f"The company {self._Companies[ID]._Name} has now closed down as it has no outlets.")

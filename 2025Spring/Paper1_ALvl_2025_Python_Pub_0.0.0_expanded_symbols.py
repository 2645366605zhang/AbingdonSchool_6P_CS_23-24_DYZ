...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):
    ...
    while not GameOver:
        ...
        if shuffleAllowed:
            UserInput = input("Enter an expression or 'SHUFFLE': ")
        ...
        if CheckIfUserInputValid(UserInput):
            ...
            if CheckNumbersUsedAreAllInNumbersAllowed(NumbersAllowed, UserInputInRPN, MaxNumber):
                ...
                if IsTarget:
                    ...
                    NumbersAllowed = FillNumbers(NumbersAllowed, TrainingGame, MaxNumber)
        elif UserInput.upper() == "SHUFFLE":
            NumbersAllowed = FillNumbers([], False, MaxNumber)
            Targets = CreateTargets([-1, -1, -1, -1, -1], TrainingGame, MaxNumber)
            Score += 1
        Score -= 1
        if Targets[0] != -1:
            GameOver = True
...
def CheckIfUserInputValid(UserInput: str):
    if re.search("^(\([0-9]+\)[\+\-\\/\^])+[0-9]+\)$", UserInput) is not None and UserInput.count("(") == UserInput.count(")"):
        return True
    else:
        return False

...
def Main():
    ...
    TrainingGame = False
    ExtraMode = None
    Choice = input("Enter y to play the training game, t to play the timed challenge mode, m to play the multiplayer mode, anything else to play a random game: ").lower()
    print()
    if Choice == "y":
        MaxNumber = 1000
        MaxTarget = 1000
        ...
        while not(Choice in ["e" ,"m", "h", "ex"]):
            Choice = input("Enter e for Easy mode, m for Medium mode, h for Hard mode, ex for Extreme mode: ").lower()
        if Choice == "e":
            MaxNumber = 6
            MaxTarget = 30
        elif Choice == "m":
    ...
    PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber, ExtraMode)
    input()
    
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber, ExtraMode):
    Score = 0
    Attempt = 0
    GameOver = False
    while not GameOver:
        ...
        if TCMode:
            print(f"Attempt Left: {20 - Attempt}")
        ...
        if CheckIfUserInputValid(UserInput):
            ...
            if CheckNumbersUsedAreAllInNumbersAllowed(NumbersAllowed, UserInputInRPN, MaxNumber):
                ...
                if IsTarget:
                    ...
                    NumbersAllowed = FillNumbers(NumbersAllowed, TrainingGame, MaxNumber)
        Score -= 1
        if Targets[0] != -1 or (TCMode and Attempt >= 20):
            GameOver = True
        else:
